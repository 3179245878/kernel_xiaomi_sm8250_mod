...
static int call_mi_disp_function(u32 fod_btn, bool from_touch)
{
    ...
    func = (void *)kallsyms_lookup_name("mi_disp_set_fod_queue_work");
    if (func) {
        pr_info("nyako_lhbm: Found symbol via kallsyms_lookup_name at %p\n", func);
        ret = func(fod_btn, from_touch);
    } else {
        pr_err("nyako_lhbm: kallsyms_lookup_name failed\n");
        ...
        if (func) {
            ...
            pr_err("nyako_lhbm: Both symbol lookup methods failed\n");
        }
    }
...
static int __init nyako_lhbm_init(void)
{
    ...
    pr_info("nyako_lhbm: kallsyms_lookup_name result: %p\n", symbol_addr);
    ...
    if (!symbol_addr) {
        pr_err("nyako_lhbm: Critical: mi_disp_set_fod_queue_work symbol not found\n");
        return -ENOENT;
    }
